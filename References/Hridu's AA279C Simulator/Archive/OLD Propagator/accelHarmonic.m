function f_sphergrav = accelHarmonic(state, state_2nd, GM, R, GMST)
...
r_vec_HCI = state(1:3) - state_2nd(1:3);
r_vec_ECI = HCItoECI(X.inc_ast, r_vec_HCI);
r_vec = ECItoECEF(GMST, r_vec_ECI);
x_pos = r_vec(1);
y_pos = r_vec(2);
z_pos = r_vec(3);
r = norm(r_vec);
...
f_ECEF = [x; y; z];
f_ECI = ECEFtoECI(GMST, f_ECEF);
f_HCI = ECItoHCI(X.inc_ast, f_ECI);
f_sphergrav = f_HCI;
end
