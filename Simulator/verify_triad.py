...
"""
Verify triad rotation matrices output
"""
...
B_body_triad_history = np.zeros((np.shape(T)[0], 3))
B_body_history = np.zeros((np.shape(T)[0], 3))
command_history = np.zeros((np.shape(T)[0], 3))
#---------------------Propagate---------------------------
t = time.time()
# M, V measurement & model matrices for triad
M = [[-0.0561111, 0.47183533], [-0.84110217,  0.39004599], [0.53795788, -0.79071838]]
V = [[-0.84639796, 0.39004599], [0.06513016, -0.47183533], [0.52855326, -0.79071838]]
...
for i, elapsed_t in enumerate(T[0:-1]):
    ...
    # Triad verification
    R = tcpp.triad_ad(M, V)
    B_body_triad = B_ECI*R
    B_body_triad_history[i+1,:] = np.transpose(B_body_triad)
...
# Triad verification
plt.figure()
plt.plot(T/3600, B_body_triad)
plt.xlabel('time [hr]')
plt.title('B_field_body w/ Triad [nT]')
plt.grid()
